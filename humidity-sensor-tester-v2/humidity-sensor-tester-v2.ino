  ...
  if (type == BME680 ) {
    ...
    while ( millis() < timeout) {
      if (Wire.available() < BME680_READ_ALL_SIZE) {
        ...
        Serial.print ("calc_hum = ");
        Serial.println (calc_hum);
        /* if (calc_hum > 100000)
          calc_hum = 100000;
          else if (calc_hum < 0)
          calc_hum = 0; */
...
void get_temperature () {
  ...
  if (type == BME680 ) {
    ...
    Serial.print("\tI2C $"); Serial.print(BME680_COEFF1_ADDR, HEX); Serial.print(" => ");
    timeout = millis() + (DEFAULT_TIMEOUT / 2);
    while ( millis() < timeout) {
      if (Wire.available() < (BME680_COEFF1_SIZE)) {
        ...
        for (int i = 0; i < BME680_COEFF1_SIZE; i++) {
          readBuffer[i] = Wire.read();
          Serial.print("0x"); Serial.print(readBuffer[i], HEX); Serial.print(", ");
        }
        Serial.println("");
    ...
    Wire.endTransmission();
    Wire.requestFrom((uint8_t)addr, (uint8_t)BME680_COEFF2_SIZE);
    Serial.print("\tI2C $"); Serial.print(BME680_COEFF2_ADDR, HEX); Serial.print(" => ");
    timeout = millis() + (DEFAULT_TIMEOUT / 2);
    while ( millis() < timeout) {
      if (Wire.available() < BME680_COEFF2_SIZE) {
        delay(BME680_MEASUREMENT_DELAY / 2);
    ...
    delay(BME680_MEASUREMENT_DELAY);
    Wire.requestFrom((uint8_t)addr, (uint8_t)BME680_READ_ALL_SIZE);
    Serial.print("\tI2C $"); Serial.print(BME680_READ_ALL, HEX); Serial.print(" => ");
    timeout = millis() + DEFAULT_TIMEOUT;
    while ( millis() < timeout) {
      if (Wire.available() < BME680_READ_ALL_SIZE) {
        delay(BME680_MEASUREMENT_DELAY);
